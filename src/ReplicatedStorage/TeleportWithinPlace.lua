...
function TeleportWithinPlace.Teleport(humanoid, params)
	...
	if humanoid.RigType == Enum.HumanoidRigType.R15 then
		...
		rootPartY = (humanoid.RootPart.Size.Y * 0.5) + humanoid.Parent.BaseColor.LeftRightToe.Size.Y + humanoid.HipHeight
	end

	-- Teleport player and request content around location if applicable
	local position = CFrame.new(params.destination + Vector3.new(0, humanoid.RootPart.Size.Y, 0))
